...
function parseGeminiJsonResponse(responseText: string): any | null {
    try {
...
router.get("/questions/:hash", async (req: Request, res: Response): Promise<void> => {
    ...
    try {
        ...
        const generationPrompt = `
          ...
          Respond ONLY with a valid JSON array containing exactly ${testDefinition.numQuestions} question objects.
          Each object in the array MUST have the following structure: {"question": "The text of the question goes here"}.
          Do NOT include any introductory text, explanations, markdown formatting like \`\`\`json, or any other text outside the JSON array itself.
        ...
        console.log(`Generating questions for test hash: ${hash}`);
        ...
        const parsedQuestions = parseGeminiJsonResponse(responseText);
        if (!parsedQuestions || !Array.isArray(parsedQuestions)) {
             console.error(`Failed to parse or validate Gemini JSON question response (hash: ${hash}). Raw text:`, responseText);
             throw new Error("AI response format was invalid (not a JSON array).");
        }
        if (parsedQuestions.length !== testDefinition.numQuestions) {
             console.warn(`Gemini returned ${parsedQuestions.length} questions, expected ${testDefinition.numQuestions} (hash: ${hash}). Using returned questions.`);
        }
        const validQuestions = parsedQuestions.filter(q => typeof q === 'object' && q !== null && typeof q.question === 'string');
        if(validQuestions.length !== parsedQuestions.length) {
            console.error(`Some items in Gemini's response were not valid question objects (hash: ${hash}). Raw parsed:`, parsedQuestions);
            throw new Error("AI response format was invalid (malformed question objects).");
        }


        console.log(`Successfully generated and parsed ${validQuestions.length} questions for test hash: ${hash}`);
        res.status(200).json({
            title: testDefinition.title, 
            subject: testDefinition.subject, 
